    ...
    <form action="{{route('admin.users.update', $user)}}" method="POST">
        ...
        <div class="space-y-4">
            <div class="grid lg:grid-cols-2 gap-4">
                ...
                <x-wire-input name="name" label="Name" required :value="old('name', $user->name)"
                ...
                <x-wire-input name="password_confirmation" label="Confirmar contraseña" type="password"
                              :value="old('password_confirm')" placeholder="Repita la contraseña" autocomplete="new-password"
                              inputmode="password" />

                <x-wire-input name="id_number" label="Numero de ID" required :value="old('id_number', $user->id_number)"
                              placeholder="Ej. 123456789" autocomplete="off" inputmode="numeric" />

                <x-wire-input name="phone" label="Telefono" required :value="old('phone', $user->phone)"
                              placeholder="Ej. 123456789" autocomplete="tel" inputmode="tel" />

            </div>

            <x-wire-input name="address" label="Direccion" required :value="old('address', $user->address)"
